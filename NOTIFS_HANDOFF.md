...
- [x] **Lune noire (Lilith)** dans le thème natal (LILITH-V1) : ~~ajouter le calcul du Mean Apogee (SE_MEAN_APOG, ipl 12) et/ou Osculating Apogee (SE_OSCU_APOG, ipl 13) dans `@astro-platform/ephemeris`.~~ → Mean Apogee branché (`lilith` dans `PLANET_IPL` swiss-engine). Mode swisseph uniquement (astracore ne peut pas calculer — fallback gracieux : positions sans `lilith`). UI : `PLANET_META` (wheel), `PLANET_COLORS` (table), labels FR/EN (prompts), glossaire — tout en place. Glyphe ⚸ et label "Lilith" déjà fournis par NatalDatasheet.
