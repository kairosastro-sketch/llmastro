...
- [ ] **Magnitude éclipse précise** : actuellement classification qualitative (`total`/`partial`/`marginal`) basée sur la distance Soleil-nœud. Pour avoir un vrai chiffre, exposer `swe_sol_eclipse_when_glob` depuis `@astro-platform/ephemeris`.
- [ ] **Détection auto-locale au signup** : actuellement default à FR, on pourrait lire `Accept-Language` header au register.
