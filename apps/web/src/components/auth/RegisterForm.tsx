...
export function RegisterForm() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!validate()) return;
    mutation.mutate({ email, password, name: name.trim() });
  };
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        <a href={authApi.googleUrl()} className="btn-ghost flex items-center justify-center gap-2 no-underline">
          <GoogleIcon />Google
        </a>
        <a href={authApi.githubUrl()} className="btn-ghost flex items-center justify-center gap-2 no-underline">
          <GitHubIcon />GitHub
        </a>
      </div>
