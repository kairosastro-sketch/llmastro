...
// Modale de confirmation de suppression d'un tirage de tarot.
// Miroir de DeleteConfirmModal (chat).
//
// UX :
//  - Overlay sombre, card centrée
//  - Titre + nom du tirage + warning + 2 boutons (Annuler + Supprimer rouge)
...
export function TarotDeleteConfirmModal({
  isOpen,
  readingTitle,
  isDeleting,
  locale,
  ...
  const readingNameStyle: CSSProperties = {
    fontFamily:    "var(--font-display, Georgia, serif)",
    fontSize:      14,
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="tarot-delete-modal-title"
    >
      <div
        style={cardStyle}
        onClick={(e) => e.stopPropagation()}
      >
        <h3 id="tarot-delete-modal-title" style={titleStyle}>
          <span aria-hidden="true">⚠</span>
          {locale === "en" ? "Delete reading?" : "Supprimer ce tirage ?"}
        </h3>

        <p style={subStyle}>
          {locale === "en"
            ? "You are about to permanently delete this saved reading:"
            : "Tu es sur le point de supprimer définitivement ce tirage sauvegardé :"}
        </p>

        <div style={readingNameStyle}>
          {readingTitle}
        </div>
