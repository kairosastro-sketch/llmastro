...
// Drawer "Mes tirages" — panneau coulissant droite.
// Miroir de ChatDrawer.
//
// Affiche la liste des tirages de tarot sauvegardés du user, avec pour chacun :
//  - le titre (cliquable pour recharger le tirage)
//  - la date de création (relative humanisée)
//  - un menu kebab ⋯ avec Renommer + Supprimer
//
// Endpoints utilisés :
//  - GET    /tarot/readings        → liste
//  - GET    /tarot/readings/:id    → détail + data (au load)
//  - PATCH  /tarot/readings/:id    → rename
//  - DELETE /tarot/readings/:id    → suppression

import { useEffect, useState } from "react";
import type { CSSProperties } from "react";
import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query";
import { apiClient } from "@/lib/api/client";
import { TarotRenameModal } from "./TarotRenameModal";
import { TarotDeleteConfirmModal } from "./TarotDeleteConfirmModal";
...
interface ReadingDetail extends ReadingListItem {
...
interface Props {
  ...
  locale:        string;
  onClose:       () => void;
  onLoadReading: (reading: LoadedReading) => void;
}
...
export function TarotDrawer({
  ...
  const { data: listRes, isLoading, isError, refetch } = useQuery({
    ...
    queryFn:  () =>
      apiClient.get<{ readings: ReadingListItem[] }>(
        "/tarot/readings",
        accessToken!,
      ),
  ...
  const readings = (listRes as { data?: { readings: ReadingListItem[] } })
    ?.data?.readings ?? [];

  // ── State : actions in-flight (load, rename, delete) ──
  const [loadingId, setLoadingId] = useState<string | null>(null);
  const [renameTarget, setRenameTarget] = useState<ReadingListItem | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<ReadingListItem | null>(null);
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      return apiClient.delete(`/tarot/readings/${id}`, undefined, accessToken!);
    },
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ["tarot-readings"] });
  ...
  // ── Handler : charger un tirage ──
  const handleLoad = async (reading: ReadingListItem) => {
    if (!accessToken) return;
    setLoadingId(reading.id);
    setActionError(null);
    try {
      const res = await apiClient.get<{ reading: ReadingDetail }>(
        `/tarot/readings/${reading.id}`,
        accessToken,
      );
      const detail = (res as { data?: { reading?: ReadingDetail } })
        ?.data?.reading;
      if (!detail || !detail.data || !Array.isArray(detail.data.cards)) {
        throw new Error("No reading data");
      }
      onLoadReading({
        readingId: detail.id,
        data:      detail.data,
  ...
  return (
    <>
      ...
      <aside
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="tarot-drawer-title"
      >
        <header style={headerStyle}>
          <h2 id="tarot-drawer-title" style={headerTitleStyle}>
            {locale === "en" ? "My readings" : "Mes tirages"}
          </h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <div style={contentStyle}>
          ...
          {!isLoading && !isError && readings.length === 0 && (
            <div style={emptyStateStyle}>
              {locale === "en"
                ? "No saved readings yet.\nUse the Save button to keep your favorite draws."
                : "Aucun tirage sauvegardé.\nUtilise le bouton Sauvegarder pour garder tes tirages favoris."}
            </div>
          )}

          {!isLoading && !isError && readings.map((reading) => {
            const isLoadingThis = loadingId === reading.id;
            ...
            return (
              ...
              >
                <button
                  type="button"
                  onClick={() => void handleLoad(reading)}
                ...
                >
                  <div style={itemRowStyle}>
                    <div style={titleStyle} title={reading.title}>
                      {reading.title}
                    </div>
                    <button
                      type="button"
                      onClick={(e) => {
                        e.stopPropagation();
                        setOpenMenuId(openMenuId === reading.id ? null : reading.id);
                      }}
                      style={kebabBtnStyle}
                  ...
                  <div style={metaStyle}>
                    <span style={badgeStyle}>
                      🃏 {locale === "en" ? "Reading" : "Tirage"}
                    </span>
                    <span>{relativeDate(reading.createdAt, locale)}</span>
                    {isLoadingThis && (
      ...
      <TarotDeleteConfirmModal
        isOpen={deleteTarget !== null}
        readingTitle={deleteTarget?.title ?? ""}
