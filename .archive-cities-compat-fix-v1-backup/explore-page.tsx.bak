...
function CompatTab() {
  ...
  const emptyAdhoc = { label: "", birthDate: "", birthTime: "", birthCity: "Paris" };
  ...
  const analyzeMutation = useMutation({
    mutationFn: async () => {
      ...
      // partnerA
      if (mode === "adhoc") {
        if (!formA.label || !formA.birthDate || !formA.birthCity) {
      ...
      if (mode === "saved") {
        ...
        if (!formB.label || !formB.birthDate || !formB.birthCity) {
          throw new Error(locale === "fr"
  ...
  const savePartnerMutation = useMutation({
    mutationFn: async () => {
      const city = CITIES[formB.birthCity];
...
function AdhocFields({
  ...
  form: { label: string; birthDate: string; birthTime: string; birthCity: string };
  setForm: React.Dispatch<React.SetStateAction<{ label: string; birthDate: string; birthTime: string; birthCity: string }>>;
  locale: string;
}) {
    ...
    <div style={{ display: "grid", gap: 8 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
        ...
        <input
          ...
          onChange={e => upd("birthTime")(e.target.value)}
        />
      </div>
      <select
        value={form.birthCity}
        onChange={e => upd("birthCity")(e.target.value)}
      >
